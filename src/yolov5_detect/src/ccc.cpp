...
//                }
        ...
        remove_duplicate(yolo_point_list_msg); //自身框去重，相当于NMS
        if (last_yolo_point_list.data.empty()) {
            ...
            for (auto now_yolo_point: yolo_point_list_msg.data) {
                ...
                for (int iter = 0; iter < (int) last_yolo_point_list.data.size(); iter++) { // 找出与last point重合度最大的point
                    auto last_yolo_point = yolo_point_list_msg.data[iter];
                    ...
                    if (overlap_rect.area() > max_overlap_area) {
                        ...
                        max_area_iter = iter;
                        std::cout << "[overlap area:" << overlap_rect.area() << "---now_rect_area:" << now_rect.area()
                        << "]" << std::endl;
                    }
                }
                if (max_overlap_area < now_rect.area() * overlap_threshold) { // now在last中无对应点，认为其新增了point
                    ...
                    if (now_id == last_id || (now_id < 12 && last_id > 11)) {
                        filter_yolo_point_list_msg.data.push_back(now_yolo_point);
//                        cv::rectangle(src_raw, cv::Rect(last_yolo_point_list.data[max_area_iter].x, last_yolo_point_list.data[max_area_iter].y,
//                                                        last_yolo_point_list.data[max_area_iter].width, last_yolo_point_list.data[max_area_iter].height),
//                                      color_table["Green"], -1);
                    ...
                    if (now_id > 11) { // now未识别出，直接用重合度最大的last进行纠正，无论last是否识别出
                        now_yolo_point = last_yolo_point_list.data[max_area_iter];
                        filter_yolo_point_list_msg.data.push_back(now_yolo_point);
//                        cv::rectangle(src_raw, cv::Rect(last_yolo_point_list.data[max_area_iter].x, last_yolo_point_list.data[max_area_iter].y,
//                                                        last_yolo_point_list.data[max_area_iter].width, last_yolo_point_list.data[max_area_iter].height),
//                                      color_table["Green"], -1);
                        last_yolo_point_list.data.erase(last_yolo_point_list.data.begin() + max_area_iter);
                    } else { // now是yolo识别出了，但是与last矛盾，进行纠正
                        if ((last_id < 6 &&  now_id > 5) || (now_id < 6 && last_id > 5)) {
                            if (diff_time_ms < time_threshold) { //两个异色且时间差较小
                                filter_yolo_point_list_msg.data.push_back(now_yolo_point);
                            }
                        } else if ((last_id < 6 && now_id < 6) || (now_id > 5 && last_id > 5)) {
                            // 两个同色
...
//                                              color_table["Green"], -1);
                ...
                ///////////////////////////////////////////////////////////////////////////////////////////
//                std::vector<std::pair<int, int>> overlap_cars;
//                for (int iter = 0; iter < (int) last_yolo_point_list.data.size(); iter++) { // 找出与last point重合度最大的point
//                    auto last_yolo_point = yolo_point_list_msg.data[iter];
//                    cv::Rect last_rect = cv::Rect(last_yolo_point.x, last_yolo_point.y, last_yolo_point.width,
//                                                  last_yolo_point.height);
//                    overlap_rect = last_rect & now_rect;
//                    if (overlap_rect.area() > now_rect.area() * overlap_threshold|| 
//                    overlap_rect.area() > last_rect.area() * overlap_threshold) {
//                        overlap_cars.push_back(std::make_pair(iter, overlap_rect.area()));
//                        std::cout << "overlap area: " << overlap_rect.area() << "---  now_rect_area" << now_rect.area() << std::endl;
//                    }
//                }
//                if (overlap_cars.empty()) { // now在last中无对应点，认为其新增了point
//                    if (now_id < 12) {
//                        filter_yolo_point_list_msg.data.push_back(now_yolo_point);
//                    } else if (now_id == 12 || now_id == 13) {
//                        // TODO: 加一个处理，给未知的预测一个值
...
//                } else { //now中某点能与last中部分点对应
//                    bool same_car = false;
//                    for (auto &i : overlap_cars) { //
//                        int last_id = last_yolo_point_list.data[i.first].id;
//                        if (now_id == last_id) {// 重叠且两个id相同，认为是同一个目标，跳过
//                            filter_yolo_point_list_msg.data.push_back(now_yolo_point);
//                            last_yolo_point_list.data.erase(last_yolo_point_list.data.begin() + i.first);
//                            same_car = true;
//                        }
//                    }
//                    if (same_car) continue; // 如果是同一辆车，已处理完，跳过now的循环
//                    std::sort(overlap_cars.begin(), overlap_cars.end(), [](const std::pair<int, int> &a, const std::pair<int, int> &b) {
//                        return a.second > b.second;
//                    });
//                    if (now_id > 11) { // now是yolo未识别出的，直接用重合度最大的进行纠正，无论last是否识别出
//                        now_yolo_point = last_yolo_point_list.data[overlap_cars[0].first];
//                    } else { // now是yolo识别出了，但是与last矛盾，进行纠正
//                        for (auto &i: overlap_cars) {
//                            int last_id = last_yolo_point_list.data[i.first].id;
//                            //TODO: last_id == 12 || 13 ?
//                            if (overlap_rate > some_value && diff_time_ms < some_value) {
//
//                            } else if () {
//
//                            }
...
//        }
        ...
        if (show_by_cv_or_msg == 1) {
            ...
            cv::resize(src_raw, src_raw, cv::Size(src_raw.cols/2, src_raw.rows/2));
...
//    else id_tracker_table = id_tracker_close_table;
    ...
    radar_interfaces::msg::YoloPoints Yolov5Detector::remove_duplicate(radar_interfaces::msg::YoloPoints& yoloPointList) {
        radar_interfaces::msg::YoloPoints preprocessed_msg;
        for (const auto& pp1 : yoloPointList.data) {
            bool overlap = false;
            for (auto& pp2 : preprocessed_msg.data) {  //检查与已经加入有没有重复，如果新的更复合要求则把已经加入队列里的替换为新的
                cv::Rect r1 = cv::Rect(pp1.x, pp1.y, pp1.width, pp1.height),
                        r2 = cv::Rect(pp2.x, pp2.y, pp2.width, pp2.height);
                cv::Rect intersect = r1 & r2;
                double overlapArea = intersect.area();
                if (pp1.id == pp2.id) {
                    ...
                    if (overlapArea > 0.3 * r1.area() || overlapArea > 0.3 * r2.area()) {
                    ...
                    if (overlapArea > 0.4 * r1.area() || overlapArea > 0.4 * r2.area()) {
                ...
                }  else if ((pp1.id < 12 && pp2.id < 12)) {
                    if (overlapArea > 0.5 * r1.area() || overlapArea > 0.5 * r2.area()) {
            ...
            if (!overlap) {
                preprocessed_msg.data.push_back(pp1);
            }
        }
