...
vector<cv::Mat> far_depth_queue;
cv::Mat far_camera_matrix = cv::Mat::zeros(3, 3, CV_64F); // 相机内参矩阵
cv::Mat far_uni_matrix = cv::Mat::zeros(3, 4, CV_64F); // 相机和雷达的变换矩阵
cv::Mat far_distortion_coefficient = cv::Mat::zeros(5, 1, CV_64F);
vector<cv::Mat> close_depth_queue;
cv::Mat close_camera_matrix = cv::Mat::zeros(3, 3, CV_64F); //　相机内参矩阵
cv::Mat close_uni_matrix = cv::Mat::zeros(3, 4, CV_64F); // 相机和雷达的变换矩阵
cv::Mat close_distortion_coefficient = cv::Mat::zeros(5, 1, CV_64F);
...
cv::Mat Cloud2Mat(const pcl::PointCloud<pcl::PointXYZI>::Ptr &input);
...
float getDepthInRect(cv::Rect rect, vector<cv::Mat> &depth_queue,  radar_interfaces::msg::YoloPoint::_id_type id);
...
void ImageSubscriber::FarImageCallback(const sensor_msgs::msg::Image::SharedPtr msg) const {
    ...
    cv::Mat src = cv_bridge::toCvShare(msg, "bgr8")->image, src_raw, src_DS;
    src.copyTo(src_raw);
    src.copyTo(src_DS);
...
//    cv::imshow("sensor_close_view", src);
    ...
    Detection_2_SortBox(src, res_cars, D_box);  // 把yolo的框转化为DS的框
    DS_far.sort(src, D_box);  //跑DS的识别
    id_process(src, D_box, res_cars);  //DS识别完了，将DS识别结果转化为yolo的框，供第二层使用

    // 展示DS的识别效果图
//    show_deep_sort(src_DS, D_box, true);
...
//            str_2_print += "r_id: " + std::to_string(real_res.class_id);
            ...
            yolo_msg.id = (uint8_t) res_cars[num_batch_count * kNumDetectBatchSize + iter_num_batch].class_id;
    ...
    // 将yolo识别的矩形发布
    if (rect_msg.data.size() != 0) {  //有识别出的矩形
        if (!camera_params_load_done_ || !pcl_load_done_) return;
        cv::Mat far_depth_calc = cv::Mat::zeros(imgRows, imgCols, CV_64FC1);//initialize the depth img 用于运算的深度图
        ...
        if (!cloud->empty()) {
            ...
            cv::Mat far_MatCloud = Cloud2Mat(cloud);  // 4*rect_msg->size
        ...
        for (int j = 0; j < (int)rect_msg.data.size(); j++) {
            radar_interfaces::msg::DistPoint point_it;
            point_it.x = rect_msg.data[j].x + rect_msg.data[j].width / 2;
            point_it.y = rect_msg.data[j].y + rect_msg.data[j].height / 2;
            point_it.dist = getDepthInRect(
                    cv::Rect(rect_msg.data[j].x, rect_msg.data[j].y, rect_msg.data[j].width, rect_msg.data[j].height),
                    far_depth_queue, rect_msg.data[j].id);
            point_it.color = rect_msg.data[j].color;
            point_it.id = rect_msg.data[j].id;
            far_distance_it.data.push_back(point_it);
            rectangle(far_depth_show,
                      cv::Rect(rect_msg.data[j].x, rect_msg.data[j].y, rect_msg.data[j].width, rect_msg.data[j].height),
                      cv::Scalar(255, 255, 255), 1);
            putText(far_depth_show, std::to_string(point_it.dist), cv::Point(point_it.x, point_it.y),
...
void ImageSubscriber::LoadCameraParams() {
...
bool FilePreparation(bool if_serialize_engine_car, bool if_serialize_engine_num, bool is_p6,
    ...
    * return_car_engine = car_engine_name;
    std::string num_wts_name = data_dir + "/weights/bestnum_5k.wts";
    std::string num_engine_name = data_dir + "/engine/bestnum_5k.engine";
