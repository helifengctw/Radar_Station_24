...
void Yolov5_Detect_Node::FarImageCallback(const sensor_msgs::msg::Image::SharedPtr msg) const {
    ...
    if (++valid_count_far >= save_threshold) {
        if (cv::imwrite("/home/hlf/Downloads/myFiles/img_car/tuned_sz/2/far/_" + std::to_string(write_count_far) + ".jpg", src)) {
            write_count_far++;
        }
        valid_count_far = 0;
    }
...
void Yolov5_Detect_Node::CloseImageCallback(const sensor_msgs::msg::Image::SharedPtr msg) const {
    ...
    if (++valid_count_close >= save_threshold) {
        if (cv::imwrite("/home/hlf/Downloads/myFiles/img_car/tuned_sz/2/close/_" + std::to_string(write_count_close) + ".jpg", src)) {
            write_count_close++;
        }
        valid_count_close = 0;
    }
