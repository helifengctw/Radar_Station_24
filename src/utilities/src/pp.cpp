...
public:
    PointsPickUp(const std::string& name) : Node(name) {
        far_img_subscription_ = this->create_subscription<sensor_msgs::msg::Image>(
                "/sensor_far/image_raw", 10, std::bind(&PointsPickUp::FarImgCallback, this, _1));
        close_img_subscription_ = this->create_subscription<sensor_msgs::msg::Image>(
                "/sensor_close/image_raw", 10, std::bind(&PointsPickUp::CloseImgCallback, this, _1));
...
int main(int argc, char ** argv)
{
    ...
    setMouseCallback("close", close_mouse_callback);
