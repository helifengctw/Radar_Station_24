...
//        frame_point_match(last_far_distance_it, far_distance_it);
//        for (auto &i: far_distance_it.data) {
//            if (i.id == 10 && i.x > 320) {
...
//        }
    ...
    resize(far_depth_show, far_depth_show, Size(720, 576));
...
//        }
    ...
    resize(close_depth_show, close_depth_show, Size(720, 576));
    imshow("close_depth_show", close_depth_show);
}
        ...
        for (int j = rect.x; j < (rect.x + rect.width); j++) {
            if (depth_queue[depth_queue.size() - 1].at<float>(i, j) > 0) {
                ...
                for (uint8_t k = 0; k < depth_queue.size(); k++) {
                    if (k < depth_queue.size() - 1 && depth_queue[k + 1].at<float>(i, j) == 0 &&
