...
bool FilePreparation(bool if_serialize_engine_car, bool if_serialize_engine_num, bool is_p6,
    ...
    * return_car_engine = car_engine_name;
    std::string num_wts_name = data_dir + "/weights/num_24_1.wts";
    std::string num_engine_name = data_dir + "/engine/num_24_1.engine";
...
//        cv::imshow("sensor_close_view", img_num_batch[0]);
...
//                cv::putText(src_raw, std::to_string((int)res_nums[m].class_id), cv::Point(number_in_img.x, number_in_img.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
            ...
            int is_blue = int (res_cars[num_batch_count * kNumDetectBatchSize + iter_num_batch].class_id == 1);
            ...
            std::cout << res_cars[num_batch_count * kNumDetectBatchSize + iter_num_batch].class_id << std::endl;
...
//                cv::putText(src_raw, std::to_string((int)res_nums[m].class_id), cv::Point(number_in_img.x, number_in_img.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
            ...
            if ((int) real_res.class_id == 14) {
                res_cars[num_batch_count * kNumDetectBatchSize + iter_num_batch].class_id = float(12 + is_blue);
//                if ((int) res_cars[num_batch_count * kNumDetectBatchSize + iter_num_batch].class_id == 0) {
//                    res_cars[num_batch_count * kNumDetectBatchSize + iter_num_batch].class_id = 12;
//                } else {
//                    res_cars[num_batch_count * kNumDetectBatchSize + iter_num_batch].class_id = 13;
//                }
            } else {
                res_cars[num_batch_count * kNumDetectBatchSize + iter_num_batch].class_id = real_res.class_id + float(6 * is_blue);
//                res_cars[num_batch_count * kNumDetectBatchSize + iter_num_batch].class_id = real_res.class_id;
            }
            //根据装甲板的rect修改车的rect
            if (real_rect.area() > 0) {
                rough_rect = real_rect;
